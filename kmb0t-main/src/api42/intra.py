...
class IntraAPIClient(object):
    ...
    def __init__(self, progress_bar=False):
        with open('data/config.yml', 'r') as cfg_stream:
            config = yaml.load(cfg_stream, Loader=yaml.BaseLoader)
            self.client_id = config['intra']['api']['client']
            self.client_secret = config['intra']['api']['secret']
            self.token_url = config['intra']['api']['uri']
            self.api_url = config['intra']['api']['endpoint']
            self.scopes = config['intra']['api']['scopes']
        ...
        while True:
                ...
                if 'www-authenticate' in res.headers:
                    ...
                    if desc == "The access token expired" or desc == "The access token is invalid":
                        if self.token != "token_dummy":
                            LOG.debug(f"Server said our token {self.token} {desc.split(' ')[-1]}")
                        if tries < 5:
            ...
            if rc >= 400:
                ...
                if rc < 500:
                    ...
                    raise ValueError(f"intra API:ClientError. Error {str(rc)}")
                else:
                    raise ValueError(f"\n{res.headers}\n\nServerError. Error {str(rc)}\n{str(res.content)}\n{req_data}")
                    raise ValueError(f"intra API:ServerError. Error {str(rc)}")
